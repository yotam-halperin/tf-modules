...
resource "aws_vpc_peering_connection" "this" {
  ...
  tags          = merge(var.tags, tomap({ "peering_side" = local.same_acount_and_region ? "both" : "requester" }))
  # hardcoded
  timeouts {
    create = "15m"
    delete = "15m"
...
resource "aws_vpc_peering_connection_accepter" "peer_accepter" {
  ...
  tags                      = merge(var.tags, tomap({ "peering_side" = local.same_acount_and_region ? "both" : "accepter" }))
}
